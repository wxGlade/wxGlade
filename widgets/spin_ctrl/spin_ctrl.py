...
class EditSpinCtrl(ManagedBase, EditStylesMixin):
    ...
    def create_widget(self):
        ...
        if self.properties["value"].is_active():
            self.widget = wx.SpinCtrl(self.parent_window2, wx.ID_ANY, min=mi, max=ma, initial=self.value)
        else:
            self.widget = wx.SpinCtrl(self.parent_window2, wx.ID_ANY, min=mi, max=ma)
