#!/usr/bin/perl -w -- 
#
# generated by wxGlade 996ac6cb8dd8+ on Tue Jan  3 14:58:14 2017
#
...
sub new {
    ...
    $wxglade_tmp_menu = Wx::Menu->new();
    $self->{mn_Unix} = Wx::MenuItem->new($wxglade_tmp_menu, wxID_ANY, _T("Unix"), _T("Use Unix line endings"), wxITEM_RADIO);
    $wxglade_tmp_menu->Append($self->{mn_Unix});
    Wx::Event::EVT_MENU($self, $self->{mn_Unix}->GetId(), $self->can('$self->{onSelectUnix}'));
    $self->{mn_Windows} = Wx::MenuItem->new($wxglade_tmp_menu, wxID_ANY, _T("Windows"), _T("Use Windows line endings"), wxITEM_RADIO);
