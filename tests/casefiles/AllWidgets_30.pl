#!/usr/bin/perl -w -- 
#
# generated by wxGlade 6d2f15a1b50c+ on Sat Jan 28 14:22:19 2017
#
...
sub new {
    ...
    $wxglade_tmp_menu = Wx::Menu->new();
    $self->{mn_Unix} = Wx::MenuItem->new($wxglade_tmp_menu, mn_IDUnix, _T("Unix"), _T("Use Unix line endings"), wxITEM_RADIO);
    $wxglade_tmp_menu->Append($self->{mn_Unix});
    Wx::Event::EVT_MENU($self, $self->{mn_Unix}->GetId(), $self->can('$self->{onSelectUnix}'));
    $self->{mn_Windows} = Wx::MenuItem->new($wxglade_tmp_menu, mn_IDWindows, _T("Windows"), _T("Use Windows line endings"), wxITEM_RADIO);
    $wxglade_tmp_menu->Append($self->{mn_Windows});
    Wx::Event::EVT_MENU($self, $self->{mn_Windows}->GetId(), $self->can('$self->{onSelectWindows}'));
    $wxglade_tmp_menu->AppendSeparator();
    $self->{mn_RemoveTabs} = Wx::MenuItem->new($wxglade_tmp_menu, wxID_ANY, _T("Remove Tabs"), _T("Remove all leading tabs"), wxITEM_CHECK);
    $wxglade_tmp_menu->Append($self->{mn_RemoveTabs});
    Wx::Event::EVT_MENU($self, $self->{mn_RemoveTabs}->GetId(), $self->can('$self->{onRemoveTabs}'));
    $wxglade_tmp_menu->AppendSeparator();
    my $wxglade_tmp_menu_sub;
    $wxglade_tmp_menu_sub = Wx::Menu->new();
    $self->{mn_RemoveWhiteSpaces} = Wx::MenuItem->new($wxglade_tmp_menu_sub, wxID_ANY, _T("Remove"), _T("Remove leading whitespace characters"), wxITEM_RADIO);
    $wxglade_tmp_menu_sub->Append($self->{mn_RemoveWhiteSpaces});
    Wx::Event::EVT_MENU($self, $self->{mn_RemoveWhiteSpaces}->GetId(), $self->can('$self->{OnRemoveWhiteSpaces}'));
    $self->{mn_KeepWhiteSpaces} = Wx::MenuItem->new($wxglade_tmp_menu_sub, wxID_ANY, _T("Keep"), _T("Don't touch leading whitespace characters"), wxITEM_RADIO);
    $wxglade_tmp_menu_sub->Append($self->{mn_KeepWhiteSpaces});
    Wx::Event::EVT_MENU($self, $self->{mn_KeepWhiteSpaces}->GetId(), $self->can('$self->{OnKeepWhiteSpaces}'));
...
sub OnRemoveWhiteSpaces {
    my ($self, $event) = @_;
    # wxGlade: All_Widgets_Frame::OnRemoveWhiteSpaces <event_handler>
    warn "Event handler (OnRemoveWhiteSpaces) not implemented";
    $event->Skip;
    # end wxGlade
}


sub OnKeepWhiteSpaces {
    my ($self, $event) = @_;
    # wxGlade: All_Widgets_Frame::OnKeepWhiteSpaces <event_handler>
    warn "Event handler (OnKeepWhiteSpaces) not implemented";
    $event->Skip;
    # end wxGlade
}
