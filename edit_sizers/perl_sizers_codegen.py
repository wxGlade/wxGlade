...
# $Id: perl_sizers_codegen.py,v 1.1 2003/06/23 22:23:20 crazyinsomniac Exp $
#
...
class PerlStaticBoxSizerBuilder:
    def get_code(self, obj):
        ...
        init = ['\t$self->{%s}= Wx::StaticBoxSizer->new(Wx::StaticBox->new(%s, -1, %s), %s);\n' %
...
class PerlGridSizerBuilder:
    ...
    def get_code(self, obj):
        ...
        init = [ '\t$self->{%s} = %s->new(%s, %s, %s, %s);\n' %
                 (obj.name, self.klass.replace('wx','Wx::',1), rows, cols, vgap, hgap) ]
        layout = []
        if obj.is_toplevel:
            layout.append('\t%s->SetAutoLayout(1)\n' % parent)
            layout.append('\t%s->SetSizer($self->{%s})\n' % (parent, obj.name))
