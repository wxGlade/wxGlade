...
class EditStylesMixin(np.PropertyOwner):
    ...
    def _set_widget_style(self):
        ...
        with self.frozen():
            ...
            old_widget = self.widget
            old_widget.Hide()
            inside_sizer = self.sizer and not self.sizer.is_virtual()
            if inside_sizer: si = self.sizer.widget.GetItem(old_widget)
            self.create_widget()
            if inside_sizer: compat.SizerItem_SetWindow(si, self.widget)
            compat.DestroyLater(old_widget)
            if inside_sizer: self.sizer.item_properties_modified(self)  # will call toplevel Refresh as well

            self.finish_widget_creation(re_add=False)
            if inside_sizer: self.sizer.layout()
            if focused:
